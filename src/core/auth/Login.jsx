...
import {
  DEFAULT_SOFTWARE_MODULE,
  SOFTWARE_MODULE_IDS,
  SOFTWARE_MODULE_OPTIONS,
} from '../config/softwareModules.js';

function Login() {
  const navigate = useNavigate();
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [selectedModule, setSelectedModule] = useState(DEFAULT_SOFTWARE_MODULE);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSoftwareModule(selectedModule);
      if (selectedModule === SOFTWARE_MODULE_IDS.backoffice) {
        navigate('/dashboard', { replace: true });
      } else {
        navigate('/module-coming-soon', { replace: true });
      }
    } catch (err) {
      const message =
    ...
    <div className="min-h-screen flex">
      ...
      >
        <div>
          ...
          <p className="text-sm mt-1 opacity-80">ERP suite</p>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-12 bg-white">
        <div className="w-full max-w-sm">
          ...
          <div className="lg:hidden text-center mb-8">
            ...
            <p className="text-sm text-gray-500 mt-1">Choose software type and sign in</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-md border border-gray-200 p-8">
            <div className="mb-8">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Select which module you are using, then sign in
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs font-medium text-gray-700" htmlFor="login-software">
                  Software type
                </label>
                <select
                  id="login-software"
                  ...
                  className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm outline-none focus:border-[#800000]"
                >
                  {SOFTWARE_MODULE_OPTIONS.map((opt) => (
