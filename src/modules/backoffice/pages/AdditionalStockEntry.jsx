...
function Toast({ msg, type }) {
  ...
  return <div className={`fixed bottom-4 right-4 z-[999] rounded px-4 py-2 text-[11px] font-semibold text-white shadow-lg ${bg}`}>{msg}</div>;
}
...
export default function AdditionalStockEntry() {
  ...
  useEffect(() => {
    ...
    const physicalUnits = baseQty + (Math.abs(parseMoneyValue(adjQty)) * parseMoneyValue(pktQty));
  ...
  const handleAdd = useCallback(() => {
    ...
    const extraAdjQty = formatQtyValue(Math.abs(parseMoneyValue(adjQty) * parseMoneyValue(pktQty)));
    const ltVal = computeLineTotalValue(extraAdjQty, pktQty);
    setTableData((prev) => [
      {
  ...
  const tableFooterRow = useMemo(() => [
    { content: <div key="ase-line-total" className="text-left font-bold">Total</div>, colSpan: 9, className: 'align-middle font-bold' },
    ...
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button type="button" className={figmaToolbarBtn} onClick={handlePost} disabled={isSaving || isPosted} aria-label="Post">
            <img src={PostIcon} alt="" className="h-3.5 w-3.5" />Post
          </button>
          <button type="button" className={figmaToolbarBtn} onClick={handleUnpost} disabled={isSaving || !isPosted} aria-label="Unpost">
            <img src={UnpostIcon} alt="" className="h-3.5 w-3.5" />Unpost
          </button>
          <button type="button" className={`${figmaToolbarBtn} font-semibold text-black`} onClick={handleDeleteDocument} disabled={isSaving} aria-label="Delete entry">
            <img src={DeleteIcon} alt="" className="h-3.5 w-3.5 brightness-0" />Delete
          </button>
          <button type="button" className={figmaToolbarBtn} onClick={handleSave} disabled={isSaving || isPosted} aria-label="Save">
            <SaveDiskIcon className="h-3.5 w-3.5 shrink-0" />{isSaving ? 'Saving…' : 'Save'}
          </button>
          <button type="button" className={primaryToolbarBtn} style={{ backgroundColor: primary, borderColor: primary }} onClick={handleNew} aria-label="New additional stock entry">
      ...
      {detailRowId && detailRow ? (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/35 px-4 py-6 backdrop-blur-sm" onClick={closeDetailModal} role="dialog" aria-modal="true" aria-labelledby="ase-line-detail-title">
          <div className="relative max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg border border-gray-200 bg-white p-4 pt-5 shadow-xl sm:max-w-lg sm:p-5 sm:pt-6" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 id="ase-line-detail-title" className="pr-10 text-sm font-bold sm:text-base" style={{ color: primary }}>Line detail</h2>
