...
function buildDummyLines(count) {
  ...
  const descs = ['Glass unit – cracked', 'Label roll – water damage', 'FG carton – crushed corner', 'Raw drum – seal broken', 'WIP batch – contamination'];
  const rows = [];
  for (let i = 0; i < count; i += 1) {
    const pkt = 1 + (i % 5);
    const present = 100 + (i * 17) % 500 + (i % 3) * 0.5;
    const adj = -Math.abs(((i % 7) - 3) * 2.5);
    ...
    rows.push({
      ...
      reason: DAMAGE_REASONS[i % DAMAGE_REASONS.length],
...
export default function DamageEntry() {
  ...
  const [damageEntryDate, setDamageEntryDate] = useState('');
  ...
  const handlePost = useCallback(() => {
    ...
    console.log('Post damage entry', { damageEntryNo, damageEntryDate, remark, tableData });
  }, [damageEntryNo, damageEntryDate, remark, tableData]);

  const handleUnpost = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Unpost damage entry', { damageEntryNo });
  }, [damageEntryNo]);

  const handleDeleteDocument = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Delete damage entry', { damageEntryNo });
  ...
  const tableFooterRow = useMemo(
    () => [
      {
        content: (
          <div key="dmg-line-total" className="text-left font-bold">
            Total
          </div>
        ),
        colSpan: 9,
    ...
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button type="button" className={figmaToolbarBtn} onClick={handlePost} aria-label="Post damage entry">
            <img src={PostIcon} alt="" className="h-3.5 w-3.5" />
            Post
          </button>
          <button type="button" className={figmaToolbarBtn} onClick={handleUnpost} aria-label="Unpost damage entry">
          ...
          <button
            ...
            aria-label="Delete damage entry"
          >
            <img src={DeleteIcon} alt="" className="h-3.5 w-3.5 brightness-0" />
            Delete
          </button>
          <button type="button" className={figmaToolbarBtn} onClick={handleSave} aria-label="Save damage entry">
      ...
      <div className="min-w-0 rounded-lg border border-gray-200 bg-white p-2 sm:p-3">
        <div className="flex min-w-0 flex-wrap items-start gap-2 sm:gap-3">
          <div className="shrink-0">
            <SubInputField
              label="Damage entry no"
              value={damageEntryNo}
              onChange={(e) => setDamageEntryNo(e.target.value)}
      ...
      {detailRowId && detailRow ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="dmg-line-detail-title"
        >
          <div
