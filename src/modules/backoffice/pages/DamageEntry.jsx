...
const DAMAGE_REASONS = [
  'Damaged', 'Expired', 'Handling / transit', 'Quality reject', 'Theft / loss', 'Other',
];
...
export default function DamageEntry() {
  ...
  const [damageEntryNo, setDamageEntryNo] = useState('');
  const [damageEntryDate, setDamageEntryDate] = useState(todayISO());
  ...
  const resetForm = useCallback(() => {
    setTableData([]); setEntryId(null); setIsPosted(false);
    setBarcode(''); setShortDescription(''); setPktQty(''); setPktDetails('');
    setSystemQty(''); setAdjQty(''); setEnteredQty(''); setPhysicalQty('');
    setReason(''); setDamageEntryNo(''); setDamageEntryDate(todayISO()); setRemark('');
    setProductIdForBarcode(null); setPage(1);
  }, []);
  ...
  useEffect(() => {
    ...
    stockApi.getEntry(Number(urlId)).then((data) => {
      ...
      setDamageEntryNo(m.entry_no ?? m.entryNo ?? '');
      setDamageEntryDate(m.entry_date ? String(m.entry_date).slice(0, 10) : todayISO());
      setRemark(m.remark ?? '');
      const lines = m.lines ?? [];
  ...
  useEffect(() => {
    const baseQty = parseMoneyValue(enteredQty) === 0 ? parseMoneyValue(systemQty) : parseMoneyValue(enteredQty);
    if (!adjQty.trim()) {
      const nextPhysicalQty = baseQty === 0 ? '' : formatQtyValue(baseQty);
      setPhysicalQty((prev) => (prev === nextPhysicalQty ? prev : nextPhysicalQty));
      return;
    }
    const physicalUnits = Math.max(0, baseQty - (Math.abs(parseMoneyValue(adjQty)) * parseMoneyValue(pktQty)));
    const nextPhysicalQty = formatQtyValue(physicalUnits);
    setPhysicalQty((prev) => (prev === nextPhysicalQty ? prev : nextPhysicalQty));
  }, [adjQty, enteredQty, pktQty, systemQty]);
  ...
  const handleAdd = useCallback(() => {
    ...
    const damageAdjQty = formatQtyValue(-Math.abs(parseMoneyValue(adjQty) * parseMoneyValue(pktQty)));
    const ltVal = computeLineTotalValue(damageAdjQty, pktQty);
    setTableData((prev) => [
      {
        id, productId: productIdForBarcode,
        barcode: barcode.trim(),
        shortDescription: shortDescription.trim() || '—',
        pktQty: pktQty.trim() || '0',
        packetDetails: packetDetailsStr,
        presentQty: systemQty.trim() || '—',
        adjustQty: damageAdjQty || '—',
  ...
  const buildPayload = useCallback(() => ({
    entryId,
    entryNo: damageEntryNo.trim() || undefined,
    docType: 'DMG',
    entryDate: damageEntryDate || todayISO(),
    remark,
    lines: tableData.map((r) => ({
      productId: r.productId ?? null, barcode: r.barcode,
      shortDescription: r.shortDescription, pktQty: r.pktQty,
      pktDetails: r.packetDetails, systemQty: r.presentQty,
      adjustQty: r.adjustQty, enteredQty: r.enteredQty,
      physicalQty: r.physicalQty, reason: r.reason, lineTotal: r.lineTotal,
    })),
  }), [entryId, damageEntryNo, damageEntryDate, remark, tableData]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (!damageEntryNo) setDamageEntryNo(result.entryNo ?? '');
      showToast('Saved successfully');
    } catch (err) {
      showToast(err?.response?.data?.message ?? 'Save failed', 'error');
    } finally { setIsSaving(false); }
  }, [buildPayload, isPosted, damageEntryNo, showToast]);
  ...
  const handleDeleteDocument = useCallback(async () => {
    if (entryId) {
      setIsSaving(true);
      try { await stockApi.deleteEntry(entryId); showToast('Deleted'); }
      catch (err) { showToast(err?.response?.data?.message ?? 'Delete failed', 'error'); setIsSaving(false); return; }
      setIsSaving(false);
    }
    resetForm();
  ...
  const tableBodyRows = useMemo(() => paginatedRows.map((r, idx) => {
    ...
    const reasonCell = rowIsEditing ? (
      <select key={`reason-${r.id}`} className={`${tableCellInputClass} max-w-full text-left`}
        value={DAMAGE_REASONS.includes(r.reason) ? r.reason : ''}
        onChange={(e) => updateLine(r.id, { reason: e.target.value })} aria-label="Reason">
        <option value="">—</option>
        {DAMAGE_REASONS.map((o) => <option key={o} value={o}>{o}</option>)}
      </select>
    ) : (r.reason || '—');
  ...
  const tableFooterRow = useMemo(() => [
    { content: <div key="dmg-line-total" className="text-left font-bold">Total</div>, colSpan: 9, className: 'align-middle font-bold' },
    ...
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          <button type="button" className={`${figmaToolbarBtn} px-2`} aria-label="Print"><img src={PrinterIcon} alt="" className="h-3.5 w-3.5" /></button>
          <button type="button" className={figmaToolbarBtn} onClick={handlePost} disabled={isSaving || isPosted} aria-label="Post damage entry">
            <img src={PostIcon} alt="" className="h-3.5 w-3.5" />Post
          </button>
          <button type="button" className={figmaToolbarBtn} onClick={handleUnpost} disabled={isSaving || !isPosted} aria-label="Unpost damage entry">
            <img src={UnpostIcon} alt="" className="h-3.5 w-3.5" />Unpost
          </button>
          <button type="button" className={`${figmaToolbarBtn} font-semibold text-black`} onClick={handleDeleteDocument} disabled={isSaving} aria-label="Delete damage entry">
            <img src={DeleteIcon} alt="" className="h-3.5 w-3.5 brightness-0" />Delete
          </button>
          <button type="button" className={figmaToolbarBtn} onClick={handleSave} disabled={isSaving || isPosted} aria-label="Save damage entry">
            <SaveDiskIcon className="h-3.5 w-3.5 shrink-0" />{isSaving ? 'Saving…' : 'Save'}
          </button>
          <button type="button" className={primaryToolbarBtn} style={{ backgroundColor: primary, borderColor: primary }} onClick={handleNew} aria-label="New damage entry">
            <PlusIcon className="h-3.5 w-3.5 shrink-0 text-white" />
            <span className="hidden sm:inline">New Damage Entry</span>
      ...
      <div className="min-w-0 rounded-lg border border-gray-200 bg-white p-2 sm:p-3">
        <div className="flex min-w-0 flex-wrap items-start gap-2 sm:gap-3">
          <div className="shrink-0">
            <SubInputField label="Damage entry no" value={damageEntryNo} onChange={(e) => setDamageEntryNo(e.target.value)} placeholder="Auto if empty" />
          </div>
          <div className="shrink-0">
            <DateInputField label="Date" value={damageEntryDate} onChange={setDamageEntryDate} />
          </div>
      ...
      <div className="min-w-0 rounded-lg border border-gray-200 bg-slate-50/70 p-2 sm:p-3">
        <div className="flex min-w-0 flex-wrap items-end gap-2 overflow-x-auto sm:gap-3">
          <div className="shrink-0">
            <SubInputField label="Barcode" value={barcode} onChange={(e) => setBarcode(e.target.value)} onKeyDown={handleBarcodeKeyDown}
              onDoubleClick={() => openProductPicker(barcode || shortDescription)} placeholder="Scan / Enter"
              inputRef={barcodeInputRef} title="Double-click to pick product" />
          </div>
          <div className="shrink-0">
          ...
          <div className="shrink-0">
            <SubInputField label="Physical qty" value={physicalQty} placeholder="Auto" type="text" inputMode="decimal" readOnly />
          </div>
          <div className="shrink-0">
            <DropdownInput label="Reason" value={reason} onChange={setReason} options={DAMAGE_REASONS} placeholder="Select" />
          </div>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <CommonTable
          className="damage-entry-table flex min-h-0 min-w-0 flex-1 flex-col"
      ...
      {detailRowId && detailRow ? (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/35 px-4 py-6 backdrop-blur-sm" onClick={closeDetailModal} role="dialog" aria-modal="true" aria-labelledby="dmg-line-detail-title">
          <div className="relative max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg border border-gray-200 bg-white p-4 pt-5 shadow-xl sm:max-w-lg sm:p-5 sm:pt-6" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 id="dmg-line-detail-title" className="pr-10 text-sm font-bold sm:text-base" style={{ color: primary }}>Line detail</h2>
