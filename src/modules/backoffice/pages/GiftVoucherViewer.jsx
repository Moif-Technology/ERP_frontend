...
const GIFT_VOUCHER_VIEWER_HEADERS = [
  ...
  'Barcode',
  'Short Description',
  <span key="h-packet-desc" className="inline-block text-center leading-tight"><span className="block whitespace-nowrap">Packet</span><span className="block whitespace-nowrap">Description</span></span>,
  <span key="h-present-qty" className="inline-block text-center leading-tight"><span className="block whitespace-nowrap">Present</span><span className="block whitespace-nowrap">Qty</span></span>,
  'Sell Price',
  'Date From',
  <span key="h-date-to" className="inline-block text-center leading-tight"><span className="block whitespace-nowrap">Date.</span><span className="block whitespace-nowrap">To</span></span>,
  'Action',
];
...
export default function GiftVoucherViewer() {
  ...
  useEffect(() => {
    ...
    api.listGiftVouchers()
      ...
      .catch((err) => { if (!cancelled) setSaveError(err?.response?.data?.message || 'Failed to load gift vouchers'); })
  ...
  const filteredRows = useMemo(() => {
    ...
    return tableData.filter((r) => {
      if (!q) return true;
      return hay(r.barcode) || hay(r.shortDescription) || hay(r.packetDescription) || hay(r.presentQty) || hay(r.sellPrice) || hay(r.dateFrom) || hay(r.dateTo);
    });
  }, [tableData, search]);
  ...
  const handleAddLine = useCallback(async () => {
    ...
    try {
      ...
      setSaveError(err?.response?.data?.message || 'Failed to add gift voucher');
    }
  }, [barcode, shortDescription, pktQty, pktDetails, sellingPrice, clearForm]);
  ...
  const tableFooterRow = useMemo(
    () => [
      { content: (<div key="gvv-total" className="text-left font-bold">Total</div>), colSpan: 4, className: 'align-middle font-bold' },
  ...
  return (
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
        ...
        <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-center sm:justify-between sm:gap-3">
          ...
          <div className="flex w-full min-w-0 flex-col gap-2 sm:h-7 sm:max-w-none sm:flex-1 sm:flex-row sm:items-center sm:justify-end sm:gap-2.5">
            <div className={figmaSearchBox}>
              ...
              <input type="search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={SEARCH_PLACEHOLDER} className="min-w-0 flex-1 border-0 bg-transparent font-['Open_Sans',sans-serif] text-[10px] font-semibold leading-5 text-black outline-none placeholder:text-neutral-400 placeholder:font-semibold" aria-label="Search table" />
            </div>
            <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5 sm:h-7 sm:flex-nowrap sm:gap-2">
              ...
              <button type="button" className={`${figmaToolbarBtn} font-semibold text-black`} onClick={handleDeleteDocument} aria-label="Delete gift voucher viewer document">
                <img src={DeleteIcon} alt="" className="h-3.5 w-3.5 brightness-0" />
                Delete
              </button>
              <button type="button" className={primaryToolbarBtn} style={{ backgroundColor: primary, borderColor: primary }} onClick={handleNewDocument} aria-label="New gift voucher viewer">
      ...
      <div className="flex min-w-0 flex-wrap items-end gap-x-2 gap-y-3 rounded-lg border border-gray-200 bg-slate-50/70 p-2 sm:p-3">
        ...
        <div className="shrink-0">
          <InputField label="short description" value={shortDescription} onChange={(e) => setShortDescription(e.target.value)} placeholder="" widthPx={140} />
        </div>
        <div className="shrink-0">
      ...
      {detailRowId && detailRow ? (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/35 px-4 py-6 backdrop-blur-sm" onClick={closeDetailModal} role="dialog" aria-modal="true" aria-labelledby="gvv-line-detail-title">
          <div className="relative max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg border border-gray-200 bg-white p-4 pt-5 shadow-xl sm:max-w-lg sm:p-5 sm:pt-6" onClick={(e) => e.stopPropagation()}>
            ...
            <h2 id="gvv-line-detail-title" className="pr-10 text-sm font-bold sm:text-base" style={{ color: primary }}>Gift voucher line</h2>
