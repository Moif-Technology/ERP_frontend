...
const STN_CODES = ['STN-HO01', 'STN-WH02', 'STN-BN03', 'STN-BS04'];
...
const PARTICULARS = [
  ...
  'Sub-lease income – warehouse mezzanine',
  'FX gain – revaluation',
  'Royalty income – brand licence',
];

const VOUCHER_TYPES = ['Goods', 'Services', 'Asset', 'Import', 'Local'];
const VOUCHER_GROUPS = ['Operating income', 'Other income', 'Interest', 'Journal', 'Adjustment'];
const POST_STATUS_OPTIONS = ['Draft', 'Posted'];

const PAGE_SIZE_OPTIONS = [10, 15, 20, 30];

/** Checkbox + Sl., Vch no, Date, Part., Type, Ref., amounts, Sts., Rmk., STN, TRN */
const IV_LIST_COL_PCT = [2, 2, 5, 6, 26, 6, 6, 6, 6, 6, 5, 5, 6, 13];

function buildDummyIncomeVouchers(count) {
  const rows = [];
  for (let i = 0; i < count; i += 1) {
    ...
    const subNum = 540 + (i * 179) % 33000 + (i % 5) * 55.5;
    const taxNum = subNum * 0.05 + (i % 3) * 8;
    ...
    rows.push({
      ...
      voucherNo: `IV-2026-${String(seq).padStart(5, '0')}`,
      voucherDate,
      particular: PARTICULARS[i % PARTICULARS.length],
      voucherType: VOUCHER_TYPES[i % VOUCHER_TYPES.length],
      refNo: `IVREF-${(68000 + i * 33) % 99000}`,
      subTotal: fmt(subNum),
      taxAmount: fmt(taxNum),
      voucherAmount: fmt(voucherNum),
      status: i % 8 === 0 ? 'Draft' : 'Posted',
      remark: i % 5 === 0 ? 'Posted to GL' : i % 5 === 1 ? 'Pending approval' : '—',
      trnNo: `260-${(290000000 + i * 100011) % 900000000}`,
    });
  }
  return rows;
}

const DUMMY_IV = buildDummyIncomeVouchers(42);
...
export default function IncomeVoucherList() {
  const [vouchers, setVouchers] = useState(() => DUMMY_IV.map((r) => ({ ...r })));
  const [search, setSearch] = useState('');
  const [dateModalOpen, setDateModalOpen] = useState(false);
  const [appliedDateRange, setAppliedDateRange] = useState(null);
  const [sortBy, setSortBy] = useState('default');
  const [filterDrawerOpen, setFilterDrawerOpen] = useState(false);
  const [ivFilters, setIvFilters] = useState({
    station: null,
    postStatus: null,
  ...
  useEffect(() => {
    setPage(1);
  }, [search, sortBy, appliedDateRange, ivFilters]);
  ...
  const activeFilterCount = useMemo(() => {
    let n = 0;
    if (ivFilters.station) n += 1;
    if (ivFilters.transactionType) n += 1;
    return n;
  }, [ivFilters.station, ivFilters.transactionType]);
  ...
  const filteredRows = useMemo(() => {
    ...
    if (ivFilters.station) {
      list = list.filter((r) => r.station === ivFilters.station);
    }
    if (ivFilters.transactionType) {
      list = list.filter((r) => r.voucherGroup === ivFilters.transactionType);
    }
    ...
    return sorted;
  }, [search, sortBy, appliedDateRange, ivFilters, vouchers]);
  ...
  const tableRows = useMemo(() => {
    ...
    const totalRow = [
      {
        content: (
          <div key="iv-list-total" className="text-left font-bold">
            Total
          </div>
        ),
        colSpan: 7,
  ...
  return (
    <div className="box-border flex min-h-0 w-[calc(100%+26px)] max-w-none flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:p-4">
      ...
      <SalesFilterDrawer
        ...
        applied={ivFilters}
        onApply={setIvFilters}
      />

      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <CommonTable
          className="income-voucher-list-table flex min-h-0 min-w-0 flex-1 flex-col"
          fitParentWidth
          allowHorizontalScroll
          truncateHeader
          truncateBody
          columnWidthPercents={IV_LIST_COL_PCT}
