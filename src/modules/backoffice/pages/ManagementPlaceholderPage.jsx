...
const TITLES = {
  ...
  '/management/branch-management':  'BRANCH MANAGEMENT',
  '/management/user-management':    'USER MANAGEMENT',
};

export default function ManagementPlaceholderPage() {
  const { pathname } = useLocation();
  const title = TITLES[pathname] || 'MANAGEMENT';
  ...
  return (
    <div className="box-border w-full min-w-0 max-w-full rounded-lg border-2 border-gray-200 bg-white p-4 shadow-sm sm:p-6">
      ...
      <p className="mt-6 text-center text-sm text-gray-600">Management screen — coming soon.</p>
    </div>
  );
}
