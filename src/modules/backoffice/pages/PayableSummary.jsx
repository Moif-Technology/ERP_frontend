...
const COMPANIES = [
  ...
  'Eastern Supplies Inc.',
  'Fresh Farm Co.',
  'Global Textiles Ltd',
  'Harbour Logistics LLC',
];
...
function buildDummyPayableRows(count) {
  const rows = [];
  for (let i = 0; i < count; i += 1) {
    const base = 800 + (i * 431) % 18000 + (i % 4) * 125.5;
    const bills = 1 + (i % 12);
    const billTotal = base * 1.15 + i * 20;
    const os = base * 0.35 + (i % 5) * 100;
    const d = 1 + (i % 26);
    const m = 1 + (i % 12);
    const slice = os / 4;
    rows.push({
      ...
      b030: fmtMoney(slice * (0.4 + (i % 3) * 0.1)),
      b3060: fmtMoney(slice * (0.25 + (i % 2) * 0.05)),
      b60120: fmtMoney(slice * (0.2)),
      b120plus: fmtMoney(slice * (0.15 + (i % 2) * 0.1)),
      managedBy: STAFF[i % STAFF.length],
      createdBy: STAFF[(i + 2) % STAFF.length],
    });
  }
  return rows;
}

const DUMMY_PAYABLE_ROWS = buildDummyPayableRows(36);
...
export default function PayableSummary() {
  const [tableData, setTableData] = useState(() => DUMMY_PAYABLE_ROWS.map((r) => ({ ...r })));
  ...
  const handleSelect = useCallback(() => {
    ...
    console.log('Payable summary select', { scope, account, station, postStatus, dateFrom, dateTo });
    setTableData(buildDummyPayableRows(36).map((r) => ({ ...r })));
    setPage(1);
  ...
  const tableFooterRow = useMemo(
    () => [
      {
        content: (
          <div key="aps-total" className="text-left font-bold">
            Total
          </div>
        ),
        colSpan: 3,
      ...
      <span key="aps-bc" className="font-bold">
        {totals.billCount}
      </span>,
      <span key="aps-bt" className="font-bold">
        {fmtMoney(totals.billTotal)}
      </span>,
      <span key="aps-os" className="font-bold">
        {fmtMoney(totals.os)}
      </span>,
      '',
      <span key="aps-30" className="font-bold">
        {fmtMoney(totals.t030)}
      </span>,
      <span key="aps-60" className="font-bold">
        {fmtMoney(totals.t3060)}
      </span>,
      <span key="aps-120" className="font-bold">
        {fmtMoney(totals.t60120)}
      </span>,
      <span key="aps-120p" className="font-bold">
  ...
  return (
    <div className="box-border flex h-full min-h-0 w-full min-w-0 max-w-full flex-1 flex-col gap-3 overflow-x-hidden rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      ...
      <div className="flex min-h-0 min-w-0 max-w-full flex-1 flex-col overflow-x-hidden">
        <CommonTable
          className="payable-summary-table flex min-h-0 min-w-0 max-w-full flex-1 flex-col overflow-x-hidden"
          fitParentWidth
      ...
      {detailRowId && detailRow ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="aps-line-detail-title"
        >
          <div
