...
const DUMMY_PAYMENT_LINES = buildDummyPaymentLines(24);
...
function buildFreshPaymentLine() {
  return {
    id: `pv-${Date.now()}`,
    account: '',
    amount: '0.00',
...
export default function PaymentVoucherEntry() {
  const [tableData, setTableData] = useState(() => DUMMY_PAYMENT_LINES.map((r) => ({ ...r })));
  ...
  const handlePost = useCallback(() => {
    ...
    console.log('Post payment voucher', { voucherType, voucherNo, tableData });
  }, [voucherType, voucherNo, tableData]);

  const handleUnpost = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Unpost payment voucher', { voucherNo });
  }, [voucherNo]);

  const handleDeleteDocument = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Delete payment voucher', { voucherNo });
    setTableData([buildFreshPaymentLine()]);
    setVoucherType('');
    setVoucherNo('');
    setAccountHead('');
    setStation('');
  ...
  const handleSave = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Save payment voucher', {
      voucherType,
      voucherNo,
      accountHead,
      station,
  ...
  }, [voucherType, voucherNo, accountHead, station, refNo, paymentDate, remark, tableData]);

  const handleNewPaymentVoucher = useCallback(() => {
    setTableData([buildFreshPaymentLine()]);
    setVoucherType('');
    setVoucherNo('');
    setAccountHead('');
  ...
  return (
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button
            ...
            aria-label="New payment voucher entry"
          >
            <PlusIcon className="h-3.5 w-3.5 shrink-0 text-white" />
            <span className="hidden sm:inline">New Payment Voucher Entry</span>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <CommonTable
          className="payment-voucher-entry-table flex min-h-0 min-w-0 flex-1 flex-col"
          fitParentWidth
