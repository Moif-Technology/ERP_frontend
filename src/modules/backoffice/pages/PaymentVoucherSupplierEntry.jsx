...
function freshLine() { return { id: `pvs-${Date.now()}-${Math.random().toString(36).slice(2)}`, accountId: '', accountLabel: '', debit: '0.00', credit: '0.00' }; }
...
export default function PaymentVoucherSupplierEntry() {
  ...
  const resetForm = useCallback(() => { setTableData([freshLine()]); setVoucherType(''); setVoucherTypeId(null); setVoucherNo(''); setLoadedVoucherId(null); setRefNo(''); setPaymentDate(''); setRemark(''); setStatusMsg(''); setPage(1); setEditingRowId(null); setDetailRowId(null); setShowList(true); }, []);
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const payload = { voucherTypeId, referenceNo: refNo || null, remarks: remark || null, voucherDate: paymentDate || undefined, lines: lines.map((l) => ({ accountId: Number(l.accountId), debitAmount: parse(l.debit), creditAmount: parse(l.credit), narration: remark || null })) };
  ...
  }, [voucherTypeId, tableData, refNo, remark, paymentDate, loadedVoucherId, loadVouchers]);
  ...
  const loadVoucherForEdit = useCallback(async (id) => {
    try {
      ...
      setPaymentDate(master.voucher_date ? master.voucher_date.slice(0, 10) : '');
      setVoucherTypeId(master.voucher_type_id);
      setVoucherType(master.voucher_name || '');
      setTableData(details.map((d) => ({ id: `pvs-${d.voucher_detail_id}`, accountId: String(d.account_id), accountLabel: d.account_no ? `${d.account_no} – ${d.account_head}` : String(d.account_id), debit: String(d.debit_amount), credit: String(d.credit_amount) })));
      setShowList(false);
      setStatusMsg('');
  ...
  return (
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        <h1 className="shrink-0 whitespace-nowrap text-sm font-bold leading-tight sm:text-base md:text-lg xl:text-xl" style={{ color: primary }}>PAYMENT VOUCHER – SUPPLIER</h1>
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button type="button" className={primaryToolbarBtn} style={{ backgroundColor: primary, borderColor: primary }} onClick={() => { resetForm(); setShowList(false); }}>
            <PlusIcon className="h-3.5 w-3.5 shrink-0 text-white" /> <span className="hidden sm:inline">New Payment Voucher</span><span className="sm:hidden">New</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex gap-2 border-b border-gray-200 pb-2">
        <button type="button" onClick={() => setShowList(true)} className={`text-[10px] font-semibold px-3 py-1 rounded-t border-b-2 transition-colors ${showList ? 'border-b-2 text-white' : 'border-transparent text-gray-600 hover:text-gray-900'}`} style={showList ? { backgroundColor: primary, borderColor: primary } : {}}>Payment Register</button>
        <button type="button" onClick={() => setShowList(false)} className={`text-[10px] font-semibold px-3 py-1 rounded-t border-b-2 transition-colors ${!showList ? 'text-white' : 'border-transparent text-gray-600 hover:text-gray-900'}`} style={!showList ? { backgroundColor: primary, borderColor: primary } : {}}>Voucher Entry</button>
      </div>
      ...
      {showList ? (
        ...
        <>
          <div className="flex min-w-0 flex-nowrap items-end gap-2 overflow-x-auto rounded-lg border border-gray-200 bg-slate-50/70 p-2 sm:gap-3 sm:p-3">
            ...
            <div className="shrink-0"><DateInputField label="Payment Date" value={paymentDate} onChange={setPaymentDate} /></div>
