...
const PRODUCT_TYPES = ['Stock', 'Non-stock', 'Service', 'Bundle', 'Raw material'];
...
function buildDummyProducts(count) {
  ...
  for (let i = 0; i < count; i += 1) {
    const base = 4.5 + (i * 37) % 120 + (i % 3) * 2.15;
    const lastPurch = base * (0.92 + (i % 5) * 0.02);
    const unitPrice = base * 1.08;
    const sell = unitPrice * (1.12 + (i % 4) * 0.03);
    const margin = sell > 0 ? ((sell - unitPrice) / sell) * 100 : 0;
    rows.push({
      id: String(i + 1),
      ownRefNo: `ORF-${String(8800 + i * 17).padStart(6, '0')}`,
      suppRefNo: `SR-${(44000 + i * 91) % 99999}`,
      ...
      pktQty: String(1 + (i % 12)),
      unitCost: fmtMoney(base),
      lastPurchCost: fmtMoney(lastPurch),
      unitPrice: fmtMoney(unitPrice),
      sellingPrice: fmtMoney(sell),
      productType: PRODUCT_TYPES[i % PRODUCT_TYPES.length],
      location: ['WH-A', 'WH-B', 'Store front', 'Back room'][i % 4],
...
const DUMMY_PRODUCTS = buildDummyProducts(52);
...
export default function ProductList() {
  const [products, setProducts] = useState(() => DUMMY_PRODUCTS.map((r) => ({ ...r })));
  ...
  const handleDeleteSelected = useCallback(() => {
    ...
    setProducts((prev) => prev.filter((row) => !ids.has(String(row.id))));
    setSelectedIds(new Set());
    setViewProduct((v) => (v && ids.has(String(v.id)) ? null : v));
  }, []);
  ...
  const filteredRows = useMemo(() => {
    ...
    let list = products.filter((r) => rowMatchesSearch(r, q));
      ...
      return [
        ...
        >
          <input
            ...
            aria-label={`Select ${r.ownRefNo}`}
          />
        </div>,
        r.ownRefNo,
        r.suppRefNo,
        r.barcode,
        <span key={`d-${r.id}`} className="block w-full text-left">
          {r.shortDescription}
        </span>,
        <span key={`s-${r.id}`} className="block w-full text-left">
          {r.supplierName}
        </span>,
        r.brand,
        r.pktQty,
  ...
  const selectedRowIndex = useMemo(() => {
    ...
    const i = paginatedRows.findIndex((r) => r.id === viewProduct.id);
    return i >= 0 ? i : null;
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:h-7 sm:shrink-0 sm:flex-nowrap">
          {selectedRowCount >= 1 ? (
            <button
              ...
              aria-label={`Delete ${selectedRowCount} selected product${selectedRowCount === 1 ? '' : 's'}`}
            >
