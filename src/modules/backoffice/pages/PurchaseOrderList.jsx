...
const LPO_LIST_COL_PCT = [12, 12, 34, 14, 12, 16];
...
function mapApiLpo(row, supplierMap) {
  const supplierId = row.supplierId != null ? String(row.supplierId) : '';
  return {
    id: String(row.lpoMasterId ?? row.lpoNo ?? `${supplierId}-${row.lpoDate}`),
    lpoNo: displayOrDash(row.lpoNo || row.lpoMasterId),
    lpoDate: formatApiDate(row.lpoDate),
    supplierName: supplierMap.get(supplierId) || (supplierId ? `Supplier ${supplierId}` : '-') ,
    lpoAmount: formatMoney(row.lpoAmount),
...
export default function PurchaseOrderList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (cancelled) return;
        setLpos((data.lpos || []).map((row) => mapApiLpo(row, supplierMap)));
      } catch (err) {
        if (!cancelled) {
          setLpos([]);
          setLoadError(err.response?.data?.message || 'Could not load LPO list.');
        }
      } finally {
        if (!cancelled) setLoading(false);
      }
  ...
  const filteredRows = useMemo(() => {
    ...
    let list = q
      ? lpos.filter(
          (r) =>
            r.lpoNo.toLowerCase().includes(q) ||
    ...
    if (appliedDateRange?.from && appliedDateRange?.to) {
      ...
      list = list.filter((r) => {
        const rd = parseListDate(r.lpoDate);
        if (!rd) return false;
        const t = rd.getTime();
        return t >= rf && t <= rt;
      });
    ...
    const sorted = [...list];
    if (sortBy === 'dateDesc') sorted.sort((a, b) => String(b.lpoDate).localeCompare(String(a.lpoDate)));
    return sorted;
  }, [search, lpos, appliedDateRange, sortBy]);
  ...
  const tableRows = useMemo(
    () =>
      paginatedRows.map((r) => [r.lpoNo, r.lpoDate, r.supplierName, r.lpoAmount, r.status, r.remarks]),
    [paginatedRows],
  );
  ...
  const emptyMessage = loading ? 'Loading LPO list...' : loadError || 'No LPOs match the current view.';
  ...
  return (
    <div className="box-border flex min-h-0 w-[calc(100%+26px)] max-w-none flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:p-4">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <CommonTable
          className="lpo-list-table flex min-h-0 min-w-0 flex-1 flex-col"
          fitParentWidth
          allowHorizontalScroll
          columnWidthPercents={LPO_LIST_COL_PCT}
          tableClassName="min-w-[min(100%,720px)] sm:min-w-[860px] lg:min-w-0"
          ...
          headers={['LPO no', 'Date', 'Supplier', 'Amount', 'Status', 'Remarks']}
          rows={tableRows}
        />
