...
const DUMMY_ACCOUNT_NAMES = [
  ...
  '2100 – Supplier control',
  '2200 – GRNI / Accrued purchases',
  '4100 – VAT input (recoverable)',
  '5100 – Purchase expense',
  '5200 – Import charges & freight',
  '5300 – Asset capitalization',
  '1200 – Bank – Main operating',
  '1300 – Petty cash',
  '6100 – Staff recovery',
];
...
export default function PurchaseVoucherEntry() {
  ...
  const handleAdd = useCallback(() => {
    ...
    const partyLabel =
      partyAcHead ||
      (PARTY_AC_HEADS[0] ?? 'New account line');
    setTableData((prev) => [
      {
        id,
  ...
  const handlePost = useCallback(() => {
    ...
    console.log('Post purchase voucher', { voucherType, voucherNo, tableData });
  }, [voucherType, voucherNo, tableData]);

  const handleUnpost = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Unpost purchase voucher', { voucherNo });
  }, [voucherNo]);

  const handleDeleteVoucher = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Delete purchase voucher', { voucherNo });
  ...
  const handleSaveVoucher = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Save purchase voucher', {
      voucherType,
      voucherNo,
      station,
      refNo,
  ...
  }, [voucherType, voucherNo, station, refNo, partyAcHead, expenseDate, remark, tableData]);

  const handleNewPurchaseVoucher = useCallback(() => {
    setTableData([buildFreshVoucherLine()]);
    setVoucherType('');
    setVoucherNo('');
    setStation('');
  ...
  const tableFooterRow = useMemo(
    () => [
      {
        content: (
          <div key="pv-line-total" className="text-left font-bold">
            Total
          </div>
        ),
        colSpan: 2,
  ...
  return (
    <div className="box-border flex h-full min-h-0 w-full min-w-0 max-w-full flex-1 flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button
            ...
            onClick={handleNewPurchaseVoucher}
            aria-label="New purchase voucher entry"
          >
            <PlusIcon className="h-3.5 w-3.5 shrink-0 text-white" />
            <span className="hidden sm:inline">New Purchase Voucher Entry</span>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <CommonTable
          className="purchase-voucher-entry-table flex min-h-0 min-w-0 flex-1 flex-col"
          fitParentWidth
      ...
      {detailRowId && detailRow ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="pv-line-detail-title"
        >
          <div
