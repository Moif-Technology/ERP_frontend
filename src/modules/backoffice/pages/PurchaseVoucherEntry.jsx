...
function freshLine() {
  return { id: `pv-${Date.now()}-${Math.random().toString(36).slice(2)}`, accountId: '', accountLabel: '', debit: '0.00', credit: '0.00' };
}

export default function PurchaseVoucherEntry() {
  ...
  const handleDeleteLine = useCallback((id) => {
    setTableData((p) => p.filter((r) => r.id !== id));
  ...
  const resetForm = useCallback(() => {
    ...
    setLoadedVoucherId(null); setRefNo(''); setPurchaseDate(''); setRemark('');
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const payload = {
        ...
        voucherDate: purchaseDate || undefined,
  ...
  }, [voucherTypeId, tableData, refNo, remark, purchaseDate, loadedVoucherId]);
      ...
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button type="button" className={primaryToolbarBtn} style={{ backgroundColor: primary, borderColor: primary }} onClick={resetForm}>
            <PlusIcon className="h-3.5 w-3.5 shrink-0 text-white" /> <span className="hidden sm:inline">New Purchase Voucher</span><span className="sm:hidden">New</span>
          </button>
        </div>
      </div>
