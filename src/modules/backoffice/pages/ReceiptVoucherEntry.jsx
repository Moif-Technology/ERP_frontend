...
const SAMPLE_ACCOUNTS = [
  ...
  '1200 – Bank – Operating',
  '1100 – Cash in hand',
  '1310 – Customer control',
];

function buildDummyReceiptLines(count) {
  const rows = [];
...
const DUMMY_RECEIPT_LINES = buildDummyReceiptLines(24);
...
function buildFreshReceiptLine() {
  return {
    id: `rv-${Date.now()}`,
    account: '',
    debit: '0.00',
...
export default function ReceiptVoucherEntry() {
  const [tableData, setTableData] = useState(() => DUMMY_RECEIPT_LINES.map((r) => ({ ...r })));
  ...
  const handlePost = useCallback(() => {
    ...
    console.log('Post receipt voucher', { voucherNo, refNo, station, receiptDate, tableData });
  }, [voucherNo, refNo, station, receiptDate, tableData]);

  const handleUnpost = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Unpost receipt voucher', { voucherNo });
  }, [voucherNo]);

  const handleDeleteDocument = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Delete receipt voucher', { voucherNo });
    setTableData([buildFreshReceiptLine()]);
    setVoucherNo('');
    setRefNo('');
    setStation('');
    setReceiptDate('');
  ...
  const handleSave = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Save receipt voucher', {
      voucherNo,
      refNo,
      station,
      receiptDate,
  ...
  }, [voucherNo, refNo, station, receiptDate, remark, tableData]);

  const handleNewReceiptVoucher = useCallback(() => {
    setTableData([buildFreshReceiptLine()]);
    setVoucherNo('');
    setRefNo('');
    setStation('');
  ...
  const tableFooterRow = useMemo(
    () => [
      {
        content: (
          <div key="rv-line-total" className="text-left font-bold">
            Total
          </div>
        ),
        colSpan: 2,
      ...
      <span key="rv-dr" className="font-bold" title="Total Dr">
        Dr {formatMoneyDisplay(totalDebit)}
      </span>,
      <span key="rv-cr" className="font-bold" title="Total Cr">
  ...
  return (
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button
            ...
            onClick={handleNewReceiptVoucher}
            aria-label="New receipt voucher entry"
          >
            <PlusIcon className="h-3.5 w-3.5 shrink-0 text-white" />
            <span className="hidden sm:inline">New Receipt Voucher Entry</span>
      ...
      <div className="flex min-w-0 flex-wrap items-end gap-x-2 gap-y-3 rounded-lg border border-gray-200 bg-slate-50/70 p-2 sm:gap-x-3 sm:gap-y-3 sm:p-3">
        ...
        <div className="shrink-0">
          <DateInputField label="Receipt Date" value={receiptDate} onChange={setReceiptDate} />
        </div>
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <CommonTable
          className="receipt-voucher-entry-table flex min-h-0 min-w-0 flex-1 flex-col"
          fitParentWidth
      ...
      {detailRowId && detailRow ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="rv-line-detail-title"
        >
          <div
