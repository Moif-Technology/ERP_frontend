...
import { DropdownInput, DateInputField } from '../../../shared/components/ui';
...
export default function ReceivableSummary() {
  ...
  const loadData = useCallback(() => {
    setLoading(true);
    setStatusMsg('');
    const params = { summaryType: 'receivable' };
    ...
    accountsApi.getAgingSummary(params)
      ...
      .catch((e) => setStatusMsg(e.response?.data?.message || 'Failed to load receivable summary'))
  ...
  return (
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        <h1 className="shrink-0 whitespace-nowrap text-sm font-bold leading-tight sm:text-base md:text-lg xl:text-xl" style={{ color: primary }}>RECEIVABLE SUMMARY</h1>
      ...
      {loading ? (
        <div className="flex flex-1 items-center justify-center">
          <p className="text-xs text-gray-500">Loading receivable summary…</p>
        </div>
      ) : (
