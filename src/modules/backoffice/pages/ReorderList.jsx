...
export default function ReorderList() {
  ...
  const headers = ['', 'Bar code', 'Short description', 'Pkt. details', 'Qty on hand', 'Unit cost', 'Reorder level', 'Reorder qty'];
  const colPct = REORDER_COL_PCT;

  const [rows, setRows] = useState(() => buildDummyReorders(48).map((r) => ({ ...r })));
  ...
  const locationOptions = STORES;
  const filterLabel = 'Store';
  ...
  const filteredRows = useMemo(() => {
    ...
    if (locationFilter) {
      list = list.filter((r) => r.store === locationFilter);
    }
  ...
  }, [paginatedRows, selectedIds, toggleRowSelected, reorderColumnTotals]);
