...
const VOUCHER_TYPES = ['Cash sale', 'Credit sale', 'Wholesale', 'Export', 'Sales return'];

const CUSTOMER_AC_HEADS = [
  '1100 – Trade receivables',
  '1200 – Customer control – local',
  '1210 – Customer control – export',
  '4100 – Sales revenue – goods',
  '4200 – Sales revenue – services',
  '2300 – VAT output (payable)',
];

const PAGE_SIZE_OPTIONS = [10, 15, 20, 30];

/** Sl no · Account name · Debit · Credit · Actions */
const VOUCHER_LINE_COL_PCT = [7, 36, 18.5, 18.5, 20];

const DUMMY_ACCOUNT_NAMES = [
  '1100 – Trade receivables',
  '1200 – Customer control',
  '4100 – Sales revenue – retail',
  '4200 – Sales revenue – wholesale',
  '2300 – VAT output (payable)',
  '4300 – Sales discounts',
  '4400 – Sales returns allowance',
  '2100 – Bank – Main operating',
  '1300 – Cash in hand',
  '5100 – Cost of sales',
];

function buildDummyVoucherLines(count) {
  const rows = [];
  for (let i = 0; i < count; i += 1) {
    const base = 850 + (i * 211) % 42000 + (i % 9) * 177.25;
    const formatted = base.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const debitRow = i % 2 === 0;
    rows.push({
      id: String(i + 1),
      accountName: DUMMY_ACCOUNT_NAMES[i % DUMMY_ACCOUNT_NAMES.length],
      debit: debitRow ? formatted : '',
      credit: debitRow ? '' : formatted,
    });
  }
  return rows;
}

const DUMMY_VOUCHER_LINES = buildDummyVoucherLines(28);
...
/** Narrow / tablet portrait: enable horizontal table scroll and safe defaults */
...
function buildFreshVoucherLine() {
  return {
    id: `sv-${Date.now()}`,
    accountName: CUSTOMER_AC_HEADS[0] ?? '',
    debit: '0.00',
    credit: '',
  };
}

function normalizeVoucherLine(r) {
  if (r.debit != null || r.credit != null) {
    return { ...r, debit: r.debit ?? '', credit: r.credit ?? '' };
  }
  const legacy = r.amount != null ? String(r.amount) : '';
  const { amount: _drop, ...rest } = r;
  return { ...rest, debit: legacy, credit: '' };
}

export default function SalesVoucherEntry() {
  const [tableData, setTableData] = useState(() => DUMMY_VOUCHER_LINES.map((r) => normalizeVoucherLine({ ...r })));
  ...
  /** Only this row shows inline editors in the table */
  const [editingRowId, setEditingRowId] = useState(null);
  /** Eye icon: modal with read-only line fields */
  ...
  const handleAdd = useCallback(() => {
    ...
    const partyLabel =
      partyAcHead ||
      (CUSTOMER_AC_HEADS[0] ?? 'New account line');
    setTableData((prev) => [
      {
        id,
        accountName: partyLabel,
        debit: '0.00',
        credit: '',
      },
      ...prev,
  ...
  const handlePost = useCallback(() => {
    ...
    console.log('Post sales voucher', { voucherType, voucherNo, tableData });
  }, [voucherType, voucherNo, tableData]);

  const handleUnpost = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Unpost sales voucher', { voucherNo });
  }, [voucherNo]);

  const handleDeleteVoucher = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Delete sales voucher', { voucherNo });
    setTableData([buildFreshVoucherLine()]);
    setVoucherType('');
    setVoucherNo('');
    setStation('');
    setRefNo('');
  ...
  const handleSaveVoucher = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Save sales voucher', {
      voucherType,
      voucherNo,
      station,
      refNo,
  ...
  }, [voucherType, voucherNo, station, refNo, partyAcHead, debitNoteDate, remark, tableData]);

  const handleNewSalesVoucher = useCallback(() => {
    setTableData([buildFreshVoucherLine()]);
    setVoucherType('');
    setVoucherNo('');
    setStation('');
  ...
  const { debitTotal, creditTotal } = useMemo(() => {
    ...
    for (const r of filteredRows) {
      d += parseMoneyValue(r.debit);
      c += parseMoneyValue(r.credit);
    }
    ...
    return paginatedRows.map((r, idx) => {
      ...
      const accountCell = rowIsEditing ? (
        <input
          ...
          value={r.accountName}
          onChange={(e) => updateLine(r.id, { accountName: e.target.value })}
  ...
  /** Pinned below data rows so N line items + total fit in one view (no total inside scroll). */
  const tableFooterRow = useMemo(
    () => [
      {
        content: (
          <div key="sv-line-total" className="text-left font-bold">
            Total
          </div>
        ),
        colSpan: 2,
    ...
    <div className="box-border flex h-full min-h-0 w-full min-w-0 max-w-full flex-1 flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button type="button" className={figmaToolbarBtn} onClick={handleSaveVoucher} aria-label="Save">
          ...
          <button
            ...
            onClick={handleNewSalesVoucher}
            aria-label="New sales voucher entry"
          >
            <PlusIcon className="h-3.5 w-3.5 shrink-0 text-white" />
            <span className="hidden sm:inline">New Sales Voucher Entry</span>
      ...
      <div className="flex min-w-0 flex-nowrap items-end gap-2 overflow-x-auto rounded-lg border border-gray-200 bg-slate-50/70 p-2 sm:gap-3 sm:p-3">
        ...
        <div className="shrink-0">
          <DateInputField label="Debit note date" value={debitNoteDate} onChange={setDebitNoteDate} />
        </div>
        <div className="shrink-0">
          <InputField
            label="Station"
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <CommonTable
          className="sales-voucher-entry-table flex min-h-0 min-w-0 flex-1 flex-col"
          fitParentWidth
          allowHorizontalScroll={isCompactTable}
          truncateHeader
          truncateBody={editingRowId == null}
          columnWidthPercents={VOUCHER_LINE_COL_PCT}
          tableClassName={isCompactTable ? 'min-w-[44rem] w-full' : 'min-w-0 w-full'}
      ...
      {detailRowId && detailRow ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="sv-line-detail-title"
        >
          <div
          ...
          >
            ...
            <div className="mt-3 flex flex-col gap-3 sm:mt-4">
              ...
              <InputField label="Account name" fullWidth readOnly value={detailRow.accountName} />
