...
function freshLine() {
  return { id: `sv-${Date.now()}-${Math.random().toString(36).slice(2)}`, accountId: '', accountLabel: '', debit: '0.00', credit: '0.00' };
}

export default function SalesVoucherEntry() {
  ...
  const resetForm = useCallback(() => {
    setTableData([freshLine()]); setVoucherType(''); setVoucherTypeId(null); setVoucherNo('');
    setLoadedVoucherId(null); setRefNo(''); setSalesDate(''); setRemark('');
    setStatusMsg(''); setPage(1); setEditingRowId(null); setDetailRowId(null);
  }, []);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      const payload = {
        voucherTypeId, referenceNo: refNo || null, remarks: remark || null,
        voucherDate: salesDate || undefined,
        lines: lines.map((l) => ({ accountId: Number(l.accountId), debitAmount: parse(l.debit), creditAmount: parse(l.credit), narration: remark || null })),
      };
      if (loadedVoucherId) {
        await accountsApi.updateVoucher(loadedVoucherId, payload);
        setStatusMsg('Voucher updated');
      } else {
        ...
        setStatusMsg('Voucher saved');
      }
    } catch (e) { setStatusMsg(e.response?.data?.message || 'Failed to save'); }
    finally { setSaving(false); }
  }, [voucherTypeId, tableData, refNo, remark, salesDate, loadedVoucherId]);

  const handlePost = useCallback(async () => {
    if (!loadedVoucherId) { setStatusMsg('Save the voucher first'); return; }
    try { await accountsApi.postVoucher(loadedVoucherId); setStatusMsg('Voucher posted'); }
  ...
  const handleUnpost = useCallback(async () => {
    ...
    try { await accountsApi.unpostVoucher(loadedVoucherId); setStatusMsg('Voucher unposted'); }
  ...
  const handleDelete = useCallback(async () => {
    if (loadedVoucherId) {
      ...
      try { await accountsApi.deleteVoucher(loadedVoucherId); }
      catch (e) { setStatusMsg(e.response?.data?.message || 'Failed to delete'); return; }
    }
    resetForm();
  }, [loadedVoucherId, resetForm]);
  ...
  const pageNumbers = useMemo(() => {
    ...
    let s = Math.max(1, page - 1); let e = Math.min(totalPages, s + 2); s = Math.max(1, e - 2);
  ...
  const tableBodyRows = useMemo(() => paginatedRows.map((r, idx) => {
    ...
    const accountCell = isEdit ? (
      <select key={`a-${r.id}`} value={r.accountId} className={`${tableCellInputClass} text-left`}
        onChange={(e) => { const acc = accountOptions.find((a) => String(a.accountId) === e.target.value); updateLine(r.id, { accountId: e.target.value, accountLabel: acc ? `${acc.accountNo} – ${acc.accountHead}` : '' }); }}>
        <option value="">— Select —</option>
        {accountOptions.map((a) => <option key={a.accountId} value={a.accountId}>{a.accountNo} – {a.accountHead}</option>)}
      </select>
    ) : <span key={`a-${r.id}`} className="block w-full text-left">{r.accountLabel || '(select account)'}</span>;
    const drCell = isEdit ? <input key={`d-${r.id}`} type="text" inputMode="decimal" className={tableCellInputClass} value={r.debit} onChange={(e) => updateLine(r.id, { debit: e.target.value })} /> : fmt(parse(r.debit));
    const crCell = isEdit ? <input key={`c-${r.id}`} type="text" inputMode="decimal" className={tableCellInputClass} value={r.credit} onChange={(e) => updateLine(r.id, { credit: e.target.value })} /> : fmt(parse(r.credit));
    return [slNo, accountCell, drCell, crCell,
  ...
  return (
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        <h1 className="shrink-0 whitespace-nowrap text-sm font-bold leading-tight sm:text-base md:text-lg xl:text-xl" style={{ color: primary }}>SALES VOUCHER ENTRY</h1>
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button type="button" className={primaryToolbarBtn} style={{ backgroundColor: primary, borderColor: primary }} onClick={resetForm}>
            <PlusIcon className="h-3.5 w-3.5 shrink-0 text-white" /> <span className="hidden sm:inline">New Sales Voucher</span><span className="sm:hidden">New</span>
          </button>
        </div>
      </div>

      {statusMsg && (
        <div className={`rounded px-3 py-1.5 text-xs font-semibold ${statusMsg.includes('saved') || statusMsg.includes('updated') || statusMsg.includes('posted') || statusMsg.includes('unposted') ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
          {statusMsg}
        </div>
      )}
      ...
      <div className="flex min-w-0 flex-nowrap items-end gap-2 overflow-x-auto rounded-lg border border-gray-200 bg-slate-50/70 p-2 sm:gap-3 sm:p-3">
        <div className="shrink-0">
          <DropdownInput label="Voucher Type" value={voucherType}
            onChange={(val) => { setVoucherType(val); const m = voucherTypes.find((t) => t.voucherName === val); setVoucherTypeId(m ? m.voucherTypeId : null); }}
        ...
        <div className="shrink-0"><DateInputField label="Sales Date" value={salesDate} onChange={setSalesDate} /></div>
      ...
      {detailRowId && detailRow && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/35 px-4 py-6 backdrop-blur-sm" onClick={closeDetailModal} role="dialog" aria-modal="true">
          <div className="relative max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg border border-gray-200 bg-white p-4 pt-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <button type="button" className="absolute right-2 top-2 inline-flex h-8 w-8 items-center justify-center rounded-md text-gray-500 hover:bg-gray-100" onClick={closeDetailModal}>
            ...
            <div className="mt-3 flex flex-col gap-3 sm:mt-4">
              ...
              <InputField label="Debit" fullWidth readOnly value={fmt(parse(detailRow.debit))} />
              <InputField label="Credit" fullWidth readOnly value={fmt(parse(detailRow.credit))} />
            </div>
          </div>
        </div>
      )}
