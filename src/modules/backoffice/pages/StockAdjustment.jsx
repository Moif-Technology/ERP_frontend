...
export default function StockAdjustment() {
  ...
  const handlePost = useCallback(() => {
    ...
    console.log('Post stock adjustment', { stockAdjNo, stockAdjDate, remark, tableData });
  }, [stockAdjNo, stockAdjDate, remark, tableData]);

  const handleUnpost = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Unpost stock adjustment', { stockAdjNo });
  }, [stockAdjNo]);

  const handleDeleteDocument = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Delete stock adjustment', { stockAdjNo });
  ...
  const handleSave = useCallback(() => {
    // eslint-disable-next-line no-console
    console.log('Save stock adjustment', {
      stockAdjNo,
      stockAdjDate,
      remark,
      lines: tableData,
  ...
  const tableFooterRow = useMemo(
    () => [
      {
        content: (
          <div key="saj-line-total" className="text-left font-bold">
            Total
          </div>
        ),
        colSpan: 9,
    ...
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      <div className="flex min-w-0 shrink-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
        ...
        <div className="flex w-full min-w-0 flex-wrap items-center justify-end gap-1.5 sm:gap-2">
          ...
          <button type="button" className={figmaToolbarBtn} onClick={handlePost} aria-label="Post stock adjustment">
            <img src={PostIcon} alt="" className="h-3.5 w-3.5" />
            Post
          </button>
          <button type="button" className={figmaToolbarBtn} onClick={handleUnpost} aria-label="Unpost stock adjustment">
          ...
          <button
            ...
            aria-label="Delete stock adjustment"
          >
            <img src={DeleteIcon} alt="" className="h-3.5 w-3.5 brightness-0" />
            Delete
          </button>
          <button type="button" className={figmaToolbarBtn} onClick={handleSave} aria-label="Save stock adjustment">
      ...
      {detailRowId && detailRow ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="saj-line-detail-title"
        >
          <div
