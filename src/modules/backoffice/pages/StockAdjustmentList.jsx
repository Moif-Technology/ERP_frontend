...
export default function StockAdjustmentList() {
  const title = 'STOCK ADJUSTMENT LIST';
  const headers = ['', 'Stock adj no', 'Post status', 'Remarks'];
  const colPct = ADJUSTMENT_COL_PCT;

  const [rows, setRows] = useState(() => buildDummyAdjustments(48).map((r) => ({ ...r })));
  ...
  const filteredRows = useMemo(() => {
    ...
    if (locationFilter) {
      list = list.filter((r) => r.brand === locationFilter);
    }
    ...
    return sorted;
  }, [search, sortBy, locationFilter, rows]);
    ...
    <div className="box-border flex min-h-0 w-[calc(100%+26px)] max-w-none flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:p-4">
      ...
      <div className="flex w-full min-w-0 flex-col gap-2 sm:h-7 sm:flex-row sm:items-center sm:justify-between sm:gap-2.5">
        <div className={figmaSearchBox}>
          ...
          <input
            ...
            placeholder="Search by adj. no, status, remarks, brand, group, supplier"
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <CommonTable
          ...
          tableClassName="min-w-[min(100%,480px)] sm:min-w-[560px] lg:min-w-0"
