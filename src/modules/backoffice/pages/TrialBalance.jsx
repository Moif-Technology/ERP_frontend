...
import { DropdownInput, InputField, DateInputField } from '../../../shared/components/ui';
...
function buildDummyTrialBalanceLines(count) {
  const rows = [];
  for (let i = 0; i < count; i += 1) {
    const base = 180 + (i * 197) % 11000 + (i % 6) * 31.5;
...
const DUMMY_TRIAL_BALANCE_LINES = buildDummyTrialBalanceLines(28);
...
export default function TrialBalance() {
  const [tableData, setTableData] = useState(() => DUMMY_TRIAL_BALANCE_LINES.map((r) => ({ ...r })));
  ...
  const handleDisplay = useCallback(() => {
    ...
    console.log('Display trial balance', { station, dateFrom, dateTo });
    setTableData(buildDummyTrialBalanceLines(28).map((r) => ({ ...r })));
    setPage(1);
  ...
  const tableFooterRow = useMemo(
    () => [
      {
        content: (
          <div key="tb-total" className="text-left font-bold">
            Total
          </div>
        ),
        colSpan: 1,
      ...
      <span key="tb-dr" className="font-bold" title="Total debit">
        Dr {formatMoneyDisplay(totalDebit)}
      </span>,
      <span key="tb-cr" className="font-bold" title="Total credit">
    ...
    <div className="box-border flex h-full min-h-0 w-[calc(100%+26px)] max-w-none min-w-0 flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:gap-4 sm:p-4">
      ...
      {detailRowId && detailRow ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="tb-line-detail-title"
        >
          <div
