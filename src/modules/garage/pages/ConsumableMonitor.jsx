...
const PARTS_LIST    = ['Engine Filter', 'Air Filter', 'Wiper Blade', 'Cabin Filter', 'Spark Plug', 'Brake Pad Set', 'Oil Drain Plug'];
...
const COL_PCT = [2, 13, 12, 24, 10, 14, 13, 12];
...
function buildRows(n) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    rows.push({
      ...
      partsNo:       `CON-${String(100 + i).padStart(3, '0')}`,
...
export default function ConsumableMonitor() {
  ...
  return (
    <div className="box-border flex min-h-0 w-[calc(100%+26px)] max-w-none flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:p-4">
      ...
      <TableRowViewModal open={Boolean(viewRow)} title="Consumable Request Detail" fields={viewModalFields} onClose={() => setViewRow(null)} />
