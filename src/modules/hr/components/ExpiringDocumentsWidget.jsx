...
import { documentAlerts } from '../data/hrData';
    ...
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden flex flex-col h-full">
      <div className="px-4 py-3 border-b border-gray-200 flex justify-between items-center bg-gray-50">
        ...
        <span className="bg-red-100 text-red-800 text-[10px] font-bold px-2 py-0.5 rounded-full">
          {documentAlerts.filter((d) => d.severity === 'High').length} Urgent
        </span>
      </div>
          ...
          {documentAlerts.map((doc) => (
            <li key={doc.id} className="p-3 bg-white border border-gray-100 rounded-md shadow-sm hover:border-gray-300 transition-colors">
              <div className="flex justify-between items-start mb-1">
                <span className="text-[11px] font-bold text-gray-800">{doc.employeeName}</span>
                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${doc.severity === 'High' ? 'bg-red-50 text-red-700 border border-red-100' : 'bg-orange-50 text-orange-700 border border-orange-100'}`}>
                  In {doc.daysLeft} days
                </span>
              </div>
              <div className="flex justify-between items-end">
                <span className="text-[10px] text-gray-500">{doc.type}</span>
                <span className="text-[10px] text-gray-500">{doc.expiryDate}</span>
              </div>
            </li>
          ))}
        </ul>
