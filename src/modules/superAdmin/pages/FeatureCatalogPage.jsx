...
export default function FeatureCatalogPage() {
  ...
  return (
    <div style={wrap}>
      <div style={header}>
        <h2 style={title}>Feature Catalog</h2>
      ...
      <div style={grid}>
          ...
          <table style={table}>
            ...
            <tbody>
              {rows.map((r) => (
                <tr
                  key={r.feature_code}
                  onClick={() => setDraft({ ...r, description: r.description || '', parent_feature_code: r.parent_feature_code || '' })}
                  style={{ cursor: 'pointer' }}
                >
                  <td style={td}>{r.feature_code}</td>
                  <td style={td}>{r.feature_name}</td>
                  <td style={td}>{r.pack_code}</td>
                  <td style={td}>{r.feature_type}</td>
                  <td style={td}>{r.is_active ? 'Yes' : 'No'}</td>
                </tr>
              ))}
              {rows.length === 0 && (
                <tr>
                  <td colSpan={5} style={{ ...td, textAlign: 'center' }}>No features found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <section style={card}>
          <h3 style={cardTitle}>Create or Update Feature</h3>
          <div style={formGrid}>
            <Field label="Feature code">
              <input style={input} value={draft.feature_code} onChange={(e) => setDraft({ ...draft, feature_code: e.target.value })} />
            </Field>
            <Field label="Feature name">
              <input style={input} value={draft.feature_name} onChange={(e) => setDraft({ ...draft, feature_name: e.target.value })} />
            </Field>
            <Field label="Pack code">
              <input style={input} value={draft.pack_code} onChange={(e) => setDraft({ ...draft, pack_code: e.target.value })} />
            </Field>
            <Field label="Parent feature">
              <input style={input} value={draft.parent_feature_code} onChange={(e) => setDraft({ ...draft, parent_feature_code: e.target.value })} />
            </Field>
            <Field label="Feature type">
              <input style={input} value={draft.feature_type} onChange={(e) => setDraft({ ...draft, feature_type: e.target.value })} />
            </Field>
            <Field label="Sort order">
              <input style={input} type="number" value={draft.sort_order} onChange={(e) => setDraft({ ...draft, sort_order: Number(e.target.value) })} />
            </Field>
