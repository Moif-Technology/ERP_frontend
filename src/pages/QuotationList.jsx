...
import QuotationDateRangeModal, { formatDDMMYYYY } from '../components/ui/QuotationDateRangeModal';
...
const figmaSearchBox =
  `flex h-7 min-h-7 w-full max-w-[200px] shrink-0 items-center gap-1 py-[3px] pl-1.5 pr-2 ${figmaOutline} sm:w-auto sm:max-w-[220px] sm:pr-3`;
    ...
    () =>
      paginatedRows.map((r) => [
        <div key={`chk-${r.id}`} className="flex justify-center">
          <input type="checkbox" className="h-3.5 w-3.5 cursor-pointer sm:h-4 sm:w-4" style={{ accentColor: primary }} aria-label={`Select ${r.quotationNo}`} />
        </div>,
        r.quotationNo,
        r.quotationDate,
        r.time,
    ...
    <div className="box-border flex min-h-0 w-[calc(100%+26px)] max-w-none flex-1 -mx-[13px] flex-col gap-3 rounded-lg border-2 border-gray-200 bg-white p-3 shadow-sm sm:p-4">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        <CommonTable
          ...
          maxVisibleRows={Math.min(pageSize, 22)}
          headers={[
            '',
            'Quotation no',
            'Quotation date',
        ...
        <div className="mt-2 flex shrink-0 flex-wrap items-center justify-between gap-2">
          ...
          <div
            className="inline-flex h-7 shrink-0 items-stretch overflow-hidden rounded-[3px] border border-gray-200 bg-white"
            role="navigation"
