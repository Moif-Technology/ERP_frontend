...
/**
 * Customer list: loyalty status filter only.
 */
export default function CustomerListFilterDrawer({
  open,
  onClose,
  onApply,
  loyaltyOptions = [],
  applied = { loyaltyStatus: null },
}) {
  const titleId = useId();
  ...
  useEffect(() => {
    if (!open) return;
    setLoyaltyStatus(applied.loyaltyStatus ?? '');
  }, [open, applied.loyaltyStatus]);
      ...
      >
        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-4 pb-4 pt-5 sm:px-5 sm:pt-6">
          ...
          <div className="flex flex-col gap-3.5 sm:gap-4">
            <div>
              <label className={labelCls} htmlFor="cl-filter-loyalty">
                Loyalty status
              </label>
              <div className={selectWrap}>
                <select
                  id="cl-filter-loyalty"
                  className={selectCls}
                  value={loyaltyStatus}
